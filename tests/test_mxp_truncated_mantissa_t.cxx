...
TEST(mxp_truncated_mantissa, add_double)
{
  ...
  Loop<0, 30, run_test_add<gt::space::host>>::Run(x, y, y_init);
}
...
TEST(mxp_truncated_mantissa, add_complex_float)
{
  ...
  const complex32_t x_init{1.f + std::exp2f(-13) + std::exp2f(-15) +
                           std::exp2f(-16)};
...
TEST(mxp_truncated_mantissa, add_complex_double)
{
  ...
  const complex64_t x_init{1. + std::exp2(-23) + std::exp2(-25) +
                           std::exp2(-26)};
...
TEST(mxp_truncated_mantissa, view_add_double)
{
  ...
  Loop<0, 30, run_test_view_add<gt::space::host>>::Run(x, y, y_init);
}
...
TEST(mxp_truncated_mantissa, view_2D_add_double)
{
  ...
  Loop<0, 30, run_test_view_2D_add<gt::space::host>>::Run(x, y, y_init);
}
...
TEST(mxp_truncated_mantissa, error_bounds_double)
{
  ...
  Loop<0, 30, run_test_error_bounds<gt::space::host>>::Run(x, y);
}
...
TEST(mxp_truncated_mantissa, device_add_double)
{
  ...
  Loop<0, 30, run_test_add<gt::space::device>>::Run(x, y, y_init);
}
...
TEST(mxp_truncated_mantissa, device_view_add_float)
{
  const int nx{3};
  const int ny{5};
  ...
  Loop<0, 23, run_test_view_add<gt::space::device>>::Run(x, y, y_init);
}

TEST(mxp_truncated_mantissa, device_view_add_double)
{
  const int nx{3};
  const int ny{5};
  ...
  const gt::gtensor_device<double, 1> x(nx, x_init);
  gt::gtensor_device<double, 1> y(ny, y_init);

  Loop<0, 30, run_test_view_add<gt::space::device>>::Run(x, y, y_init);
}

TEST(mxp_truncated_mantissa, device_view_add_complex_float)
{
  using complex32_t = gt::complex<float>;
...
TEST(mxp_truncated_mantissa, device_view_add_complex_double)
{
  using complex64_t = gt::complex<double>;
  ...
  const gt::gtensor_device<complex64_t, 1> x(nx, x_init);
  gt::gtensor_device<complex64_t, 1> y(ny, y_init);

  Loop<0, 30, run_test_view_add<gt::space::device>>::Run(x, y, y_init);
}

TEST(mxp_truncated_mantissa, device_view_2D_add_float)
{
  const int nx{3};
  const int mny[2]{5, 2};
  ...
  const gt::gtensor_device<float, 1> x(nx, x_init);
  gt::gtensor_device<float, 2> y(gt::shape(mny[0], mny[1]), y_init);

  Loop<0, 23, run_test_view_2D_add<gt::space::device>>::Run(x, y, y_init);
}

TEST(mxp_truncated_mantissa, device_view_2D_add_double)
{
  const int nx{3};
  const int mny[2]{5, 2};
  ...
  const gt::gtensor_device<double, 1> x(nx, x_init);
  gt::gtensor_device<double, 2> y(gt::shape(mny[0], mny[1]), y_init);

  Loop<0, 30, run_test_view_2D_add<gt::space::device>>::Run(x, y, y_init);
}

TEST(mxp_truncated_mantissa, device_view_2D_add_complex_float)
{
  using complex32_t = gt::complex<float>;
  ...
  const gt::gtensor_device<complex32_t, 1> x(nx, x_init);
  gt::gtensor_device<complex32_t, 2> y(gt::shape(mny[0], mny[1]), y_init);

  Loop<0, 23, run_test_view_2D_add<gt::space::device>>::Run(x, y, y_init);
}

TEST(mxp_truncated_mantissa, device_view_2D_add_complex_double)
{
  using complex64_t = gt::complex<double>;
  ...
  const gt::gtensor_device<complex64_t, 1> x(nx, x_init);
  gt::gtensor_device<complex64_t, 2> y(gt::shape(mny[0], mny[1]), y_init);
...
TEST(mxp_truncated_mantissa, device_error_bounds_double)
{
  ...
  Loop<0, 30, run_test_error_bounds<gt::space::device>>::Run(x, y);
}
