...
void generic_axaxaxpy_explicit_mxp(const T a, const gt::gtensor<T, 1, S>& x,
                                   gt::gtensor<T, 1, S>& y)
{
  const auto mxp_x = gt::mxp_adapt<1, S, X>(x.data(), x.shape());
  auto mxp_y = gt::mxp_adapt<1, S, X>(y.data(), y.shape());
...
template <typename S, typename X, typename T>
void generic_aXaXaXpy_2D_explicit_mxp(const T a, const gt::gtensor<T, 2, S>& xx,
                                      gt::gtensor<T, 2, S>& yy)
{
  const auto mxp_X = gt::mxp_adapt<2, S, X>(xx.data(), xx.shape());
  auto mxp_Y = gt::mxp_adapt<2, S, X>(yy.data(), yy.shape());
...
template <typename S, typename X, typename T>
void generic_complex_axaxaxpy_explicit_mxp(
  const gt::gtensor<T, 1, S>& a, const gt::gtensor<gt::complex<T>, 1, S>& x,
  gt::gtensor<gt::complex<T>, 1, S>& y)
{
  const auto mxp_a = gt::mxp_adapt<1, S, X>(a.data(), a.shape());
  const auto mxp_x = gt::mxp_adapt<1, S, X>(x.data(), x.shape());
  auto mxp_y = gt::mxp_adapt<1, S, X>(y.data(), y.shape());
...
template <typename S, typename X, typename T>
void generic_complex_op_plus_explicit_mxp(
  const gt::gtensor<gt::complex<T>, 1, S>& x,
  gt::gtensor<gt::complex<T>, 1, S>& y)
{
  const auto mxp_x = gt::mxp_adapt<1, S, X>(x.data(), x.shape());
...
template <typename S, typename X, typename T>
void generic_complex_op_minus_explicit_mxp(
  const gt::gtensor<gt::complex<T>, 1, S>& x,
  gt::gtensor<gt::complex<T>, 1, S>& y)
{
  const auto mxp_x = gt::mxp_adapt<1, S, X>(x.data(), x.shape());
...
template <typename S, typename T>
void generic_complex_op_multiply_explicit_gt(
  const gt::gtensor<gt::complex<T>, 1, S>& x,
  gt::gtensor<gt::complex<T>, 1, S>& y)
{
  const auto gt_x = gt::adapt<1, S>(x.data(), x.shape());
  ...
  gt::launch<1, S>(
    y.shape(), GT_LAMBDA(int j) { gt_y(j) = gt_x(j) * gt_x(j) * gt_x(j); });
}

template <typename S, typename X, typename T>
void generic_complex_op_multiply_explicit_mxp(
  const gt::gtensor<gt::complex<T>, 1, S>& x,
  gt::gtensor<gt::complex<T>, 1, S>& y)
{
  const auto mxp_x = gt::mxp_adapt<1, S, X>(x.data(), x.shape());
  ...
  gt::launch<1, S>(
    y.shape(), GT_LAMBDA(int j) { mxp_y(j) = mxp_x(j) * mxp_x(j) * mxp_x(j); });
}
...
TEST(mxp, complex_op_multiply_explicit)
{
  ...
  generic_complex_op_multiply_explicit_gt<gt::space::host>(x, y);

  EXPECT_EQ(y, (gt::gtensor<complex32_t, 1>(n, gt_ref)));

  generic_complex_op_multiply_explicit_mxp<gt::space::host, complex64_t>(x, y);
...
TEST(mxp, complex_op_divide_implicit)
{
  ...
  const auto mxp_x = gt::mxp_adapt<1, complex64_t>(x.data(), x.size());
  auto mxp_y = gt::mxp_adapt<1, complex64_t>(y_b.data(), y_b.size());
...
template <typename S, typename T>
void generic_complex_op_divide_explicit_gt(
  const gt::gtensor<gt::complex<T>, 1, S>& x,
  gt::gtensor<gt::complex<T>, 1, S>& y)
{
  const auto gt_x = gt::adapt<1, S>(x.data(), x.shape());
  ...
  gt::launch<1, S>(
    y.shape(), GT_LAMBDA(int j) { gt_y(j) = gt_y(j) / gt_x(j) / gt_x(j); });
}

template <typename S, typename X, typename T>
void generic_complex_op_divide_explicit_mxp(
  const gt::gtensor<gt::complex<T>, 1, S>& x,
  gt::gtensor<gt::complex<T>, 1, S>& y)
{
  const auto mxp_x = gt::mxp_adapt<1, S, X>(x.data(), x.shape());
  ...
  gt::launch<1, S>(
    y.shape(), GT_LAMBDA(int j) { mxp_y(j) = mxp_y(j) / mxp_x(j) / mxp_x(j); });
}
...
TEST(mxp, complex_op_divide_explicit)
{
  ...
  generic_complex_op_divide_explicit_gt<gt::space::host>(x, y_a);

  double gt_err = std::abs(y_a[1].real() - ref);
  EXPECT_GT(gt_err, lb_err_expect_gt);

  generic_complex_op_divide_explicit_mxp<gt::space::host, complex64_t>(x, y_b);
...
TEST(mxp, view_placeholders_complex_aXaXaXpY_2D)
{
  ...
  auto mxp_Y = gt::mxp_adapt<2, complex64_t>(Y.data(), mn);
...
TEST(mxp, device_axaxaxpy_implicit)
{
  ...
  auto mxp_y =
    gt::mxp_adapt_device<1, double>(gt::raw_pointer_cast(y.data()), y.size());
...
TEST(mxp, device_aXaXaXpY_2D_implicit)
{
  ...
  auto mxp_Y =
    gt::mxp_adapt_device<2, double>(gt::raw_pointer_cast(Y.data()), mn);
...
TEST(mxp, device_complex_axaxaxpy_implicit)
{
  using complex32_t = gt::complex<float>;
  using complex64_t = gt::complex<double>;
  ...
  auto mxp_y = gt::mxp_adapt_device<1, complex64_t>(
    gt::raw_pointer_cast(y.data()), y.size());

  mxp_y = mxp_y + mxp_a * mxp_x + mxp_a * mxp_x + mxp_a * mxp_x;

  EXPECT_EQ(y, (gt::gtensor_device<complex32_t, 1>(n, y_init + x_init)));
}

TEST(mxp, device_complex_axaxaxpy_explicit)
{
  using complex32_t = gt::complex<float>;
  using complex64_t = gt::complex<double>;
...
TEST(mxp, device_complex_op_plus_implicit)
{
  using complex32_t = gt::complex<float>;
  using complex64_t = gt::complex<double>;
...
TEST(mxp, device_complex_op_plus_explicit)
{
  using complex32_t = gt::complex<float>;
  using complex64_t = gt::complex<double>;
...
TEST(mxp, device_complex_op_minus_implicit)
{
  using complex32_t = gt::complex<float>;
  using complex64_t = gt::complex<double>;
...
TEST(mxp, device_complex_op_minus_explicit)
{
  using complex32_t = gt::complex<float>;
  using complex64_t = gt::complex<double>;
...
TEST(mxp, device_complex_op_multiply_implicit)
{
  using complex32_t = gt::complex<float>;
  using complex64_t = gt::complex<double>;
  ...
  EXPECT_EQ(y, (gt::gtensor_device<complex32_t, 1>(n, gt_ref)));
...
TEST(mxp, device_complex_op_multiply_explicit)
{
  using complex32_t = gt::complex<float>;
  using complex64_t = gt::complex<double>;
  ...
  generic_complex_op_multiply_explicit_gt<gt::space::device>(x, y);

  EXPECT_EQ(y, (gt::gtensor_device<complex32_t, 1>(n, gt_ref)));

  generic_complex_op_multiply_explicit_mxp<gt::space::device, complex64_t>(x,
                                                                           y);

  EXPECT_EQ(y, (gt::gtensor_device<complex32_t, 1>(n, mxp_ref)));
}

TEST(mxp, device_complex_op_divide_implicit)
{
  using complex32_t = gt::complex<float>;
  using complex64_t = gt::complex<double>;
  ...
  const gt::gtensor_device<complex32_t, 1> x(n, x_init);
  gt::gtensor_device<complex32_t, 1> y_a(n, y_init);
  gt::gtensor_device<complex32_t, 1> y_b(n, y_init);

  const auto gt_x =
    gt::adapt_device<1>(gt::raw_pointer_cast(x.data()), x.size());
  auto gt_y = gt::adapt_device<1>(gt::raw_pointer_cast(y_a.data()), y_a.size());

  gt_y = gt_y / gt_x / gt_x;

  gt::gtensor<complex32_t, 1> h_y(n);
  gt::copy(y_a, h_y);
...
TEST(mxp, device_complex_op_divide_explicit)
{
  using complex32_t = gt::complex<float>;
  using complex64_t = gt::complex<double>;
  ...
  generic_complex_op_divide_explicit_gt<gt::space::device>(x, y_a);
  ...
  generic_complex_op_divide_explicit_mxp<gt::space::device, complex64_t>(x,
                                                                         y_b);
...
TEST(mxp, device_view_s_2D)
{
  ...
  EXPECT_EQ(Y, (gt::gtensor_device<float, 2>{
                 {y_init, y_init, y_init, y_init},
                 {y_init, y_init, y_init + x_init, y_init + x_init},
                 {y_init, y_init, y_init + x_init, y_init + x_init}}));
}

TEST(mxp, device_view_slice_2D)
{
  const int mn[2]{4, 3};
  const int slice_idx = 1;
  ...
  EXPECT_EQ(Y,
            (gt::gtensor_device<float, 2>{{y_init, y_init, y_init, y_init},
                                          {y_init + x_init, y_init + x_init,
                                           y_init + x_init, y_init + x_init},
                                          {y_init, y_init, y_init, y_init}}));
}

TEST(mxp, device_view_view_axaxaxpy)
{
  const int nx{3};
  const int ny{5};
  const float x_init{std::exp2f(-23)};
...
TEST(mxp, device_view_placeholders_complex_aXaXaXpY_2D)
{
  ...
  const auto mxp_x =
    gt::mxp_adapt_device<1, complex64_t>(gt::raw_pointer_cast(x.data()), mn[0]);
  auto mxp_Y =
    gt::mxp_adapt_device<2, complex64_t>(gt::raw_pointer_cast(Y.data()), mn);
